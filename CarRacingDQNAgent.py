...
class CarRacingDQNAgent:
    ...
    def build_model(self):
        ...
        model.add(tf.keras.layers.Conv2D(filters=6, kernel_size=(7, 7), strides=3,
                  activation='relu', input_shape=(96, 96, self.frame_stack_num)))
        ...
        model.compile(loss='mean_squared_error', optimizer=tf.keras.optimizers.Adam(
            lr=self.learning_rate, epsilon=1e-7))
        return model
    ...
    def replay(self, batch_size):
        ...
        self.model.fit(np.array(train_state), np.array(
            train_target), epochs=1, verbose=0)
