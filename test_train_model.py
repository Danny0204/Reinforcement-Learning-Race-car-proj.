...
RENDER = True
STARTING_EPISODE = 1
ENDING_EPISODE = 5000#5000
SKIP_FRAMES = 2
TRAINING_BATCH_SIZE = 64
SAVE_TRAINING_FREQUENCY = 100#100
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '-m', '--model', help='Specify the last trained model path if you want to continue training after it.')
    ...
    for e in range(STARTING_EPISODE, ENDING_EPISODE+1):
        ...
        while True:
            ...
            agent.memorize(current_state_frame_stack, action,
                           reward, next_state_frame_stack, done)

            if done or negative_reward_counter >= 25 or total_reward < 0:
                print('Episode: {}/{}, Scores(Time Frames): {}, Total Rewards(adjusted): {:.2}, Epsilon: {:.2}'.format(
        ...
        if e % SAVE_TRAINING_FREQUENCY == 0:
            ...
            agent.save('./save_test/trial_{}.h5'.format(e))
    ...
    plt.plot(range(STARTING_EPISODE, ENDING_EPISODE+1), scores)
    plt.xlabel('Episode')
    plt.ylabel('Scores (Time Frames)')
    plt.title('Training Scores')
    plt.show()

    plt.plot(range(STARTING_EPISODE, ENDING_EPISODE+1), rewards)
    plt.xlabel('Episode')
    plt.ylabel('Total Rewards (adjusted)')
    plt.title('Training Rewards')
    plt.show()

    plt.plot(range(STARTING_EPISODE, ENDING_EPISODE+1), epsilons)
    plt.xlabel('Episode')
    plt.ylabel('Epsilon')
